...
import { serviceQualityPillars } from "./homeData";
    ...
    <section ref={ref} className="border-y border-[#00D9FF]/18 bg-[#06142E] px-5 py-16 text-[#F5FCFF] md:px-10 lg:px-14">
      <div className="mx-auto max-w-[1540px]">
        <div className="grid gap-8 md:grid-cols-[0.62fr_1.38fr] md:items-center">
          <div>
            <p className="terminal-label">Compromiso operativo</p>
          ...
          <div className="grid gap-px overflow-hidden rounded-md border border-white/12 bg-white/12 md:grid-cols-4">
            {serviceQualityPillars.map((pillar, index) => (
              <article className="bg-[#031B3A] p-5" data-metric-card key={pillar.title}>
                <p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--color-brand-lime)]">
                  {String(index + 1).padStart(2, "0")}
                </p>
                <h3 className="mt-10 text-lg font-semibold uppercase leading-tight">{pillar.title}</h3>
