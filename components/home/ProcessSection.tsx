...
export function ProcessSection() {
  ...
  return (
    <section id="proceso" ref={sectionRef} className="relative h-[420svh] bg-[#07101f] text-white md:h-[440vh]">
      <div className="sticky top-0 mx-auto grid h-[100svh] max-w-[1540px] grid-rows-[auto_1fr] gap-5 px-5 pb-8 pt-24 md:h-screen md:grid-cols-[0.78fr_1.22fr] md:grid-rows-1 md:items-center md:gap-10 md:px-10 md:py-16 lg:px-14">
        <div className="relative z-10">
          ...
          <div className="mt-4 overflow-hidden md:mt-8">
            <p className="process-number text-[clamp(3.2rem,9vw,9.5rem)] font-semibold leading-none tracking-[-0.04em] text-white/18">
              {active.number}
            </p>
          </div>
          ...
          <div className="mt-5 flex gap-3">
            {processSteps.map((step, index) => (
              <button
                className={`h-2 flex-1 rounded-full transition ${activeStep === index ? "bg-white" : "bg-white/18"}`}
                key={step.number}
                type="button"
        ...
        <div className="relative min-h-0 overflow-hidden rounded-md border border-white/12 bg-white/6 md:min-h-[460px]" data-process-step={active.number}>
          {processSteps.map((step, index) => (
            <Image
              key={step.number}
              src={step.frame}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#07101f]/72 via-transparent to-transparent" />
